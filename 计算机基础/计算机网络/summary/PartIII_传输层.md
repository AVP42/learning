...
4. 为什么TCP是可靠的，如何实现
   ...
   * 如何保证
   ...
   5. TCP可靠传输的实现细节
      ...
      * **超时重传时间如何选择**
         ...
         * 往返时延方差大： 由于IP数据报所选择的路由变化很大，所以往返时间RTT的方差比较大
         ...
         >
         > 3. 如果重传了的不作为报文，如果网络时延增大了，导致一直接收不到确认，从而一直重试，而重传时间也得不到更新的死循环境地。
         >
   ...
   6. **TCP的流量控制**
      ...
      需要注意的是：当接收方的窗口从0恢复到可以接收数据时，可能出现死锁。因为后一次的ACK可能会丢失。
      ...
      * 【主动】通过将PUSH标志位置1，实现推送的效果
      ...
      >
      > 可以用来解决**发送方的糊涂窗口综合征**，即每次只发送一个字节的有效数据
   ...
   7. **TCP的拥塞控制**
      ...
      > 拥塞控制包括了流量控制？，流量控制的背压机制只考虑**接收方**的情况，未考虑整条**全局**链路。
      ...
      * ==**TCP 拥塞控制**==
        ...
        * 控制过程----可能发生了拥塞
          ...
          * 可能发生了拥塞的情况----收到基于快重传算法的3次重复的ACK
            ...
            * **快重传  fast retransmission** 要求接收方不要等待自己发送数据的时候才捎带确认消息，而是要立即发送确认，即时收到了失序的报文，也要立即发出对已收到报文段的确认
   ...
   8. AQM (Active Queue Management) 主动队列管理
      ...
      * AQM的实现：
        ...
        * 实现1：随机早期检测 RED (Random Early Detection)   效果不是太理想，2015年deprecated
          ...
          对每一个到达时的分组都会计算队列的平均长度L

          * 当L大于队列长度最小门限，直接进入队列排队
   ...
   9. TCP 报文段segment格式
      ...
      * 首部：
        ...
        * 固定20个字节 + 4n字节拓展选项。即最小长度是20
          ...
          | 数据偏移offset    | 0.5字节              | 即首部长度，因为首部可拓展，所以使用offset来标记数据部分距离报文起始处的距离。<br />单位是4字节，如果offset是32，说明从128开始才是数据部分 |
          ...
          | window            | 2字节                | 窗口，用来给对方设置发送窗口的依据，单位是字节。<br />window=8，表示对方应当设置8字节的窗口。最大是64K字节<br />窗口值时动态变化的 |
...
* TCP 连接队列
  ...
  ![åè¿æ¥éåä¸å¨è¿æ¥éå](images/3.jpg)

  ![image-20211128150702956](images/image-20211128150702956.png)

  ![image-20211128163405421](images/image-20211128163405421.png)
...
* TCP服务器的最大并发连接数
  ...
  * TCP协议并没有限制，是linux的文件描述符数量限制了端口的数量，从而限制了连接数，理论上最大并发连接数为65535。可以通过ulimit -n xxx进行修改。
...
* TCP 4次挥手
  ...
  * 为什么需要第四次挥手
    ...
    如果没有这个ACK，服务器直接进入关闭状态，而A没有收到FIN+ACK报文，以为B还有数据要发送，就不会关闭资源。所以B必须保持可重传FIN的状态。
...
* TCP 如何保证可靠性的
  ...
  * 流控
    ...
    通过Window 窗口设置来解决流控问题

    * 慢启动

      > window 有Advertised Window，Sliding Window， Congestion Window

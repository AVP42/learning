...
//            mid = (l + r) >> 1;
    ...
    /** 所以对于000111类型找第一个1，mid 靠左，
     *      使用哨兵， 判断mid元素，结果为l处，当l==nums.length说明不存在；
     *      不使用哨兵，判断mid元素, 则需要额外判定l==r=nums.length -1时的元素
     * 所以对于111000类型找最后一个1，mid 靠右，
     *      使用哨兵，判断mid - 1元素，结果为l - 1处, 当l==-1时，说明不存在；
     *      不使用哨兵，判断mid元素, 则需要额外判定l==r=0时的元素
     *
     * 也不需要实际死记硬背，11110000可以转成00000111(可以用上面的第一种方法), 注意判断是反过来的，找到第一个1之后，ans - 1的位置就是第一个0的位置，ans-1=-1表示不存在。
     *
     * 还可以使用万能变种，不使用哨兵，使用变量存储结果，在为1的时候存储，判断mid元素，l=mid+1, r=mid-1。如果不存在，则为-1.
     */

    // 00001111类型  mid靠左（靠近0的那边）
    //            使用哨兵    不使用哨兵(需要判断边界)
    // 11111返回     0          0
    // 00000返回     5          4
    // 00001返回     4          4

 ...
 *                  * 钩子方法命名应当以“do”作为前缀
 *     3.在重构中的应用：
 *              以多态性取代条件转移：即模版方法中如果有条件选择，可以将条件选择下沉至子类进行，模版方法就可以不关心条件逻辑，转移到子类
 *          重构原则：对于一个继承的等级结构进行重构时，将行为移动到结构顶端，将状态移动到结构低端
 *              1. 应该根据行为来定义一个类，而不是状态。即一个类的实现首先应当建立在行为的基础之上，而不是状态智商
 *              2. 在实现行为时，应当使用抽象状态，而不是具体状态。如果行为涉及到使用状态，应当使用间接引用，而不是直接引用。也就是通过取值的方法，而不是直接引用变量

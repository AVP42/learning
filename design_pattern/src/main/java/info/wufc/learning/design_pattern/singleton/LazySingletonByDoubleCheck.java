 ...
 *   <p>因为java规范中的规范 允许单线程内不改变线程执行结果的重排序，所以2和3步操作可能重排序为先3后2；
 *   <p>这时如果<B>另外的一个线程</B>B线程在A线程刚执行完3的时候，判断到变量不为空，随机返回，但是对象还没真正初始化,
 *   导致拿到可能是不完整的对象。
 *
 *  <p>而使用了volatile之后，volatile提供了两个功能，保证线程可见性以及提供一定的有序性。

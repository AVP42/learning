 ...
 *          使用访问者模式，遵循了对算法变化的封装，但是没有遵循对数据结构中元素类型变化的封装。
 ...
 *                  对象数结构的设计可以采用合成模式实现。
 *          5.3 实现时，应当尽可能多的将对象浏览逻辑放在Visitor类中，而不是他的子类，这样ConcreteVisitor对所访问的对象结构依赖较少，维护更加方便
 *          5.4 示例中由结构对象来遍历，我们可以使用一个迭代对象来遍历，甚至交给访问者来遍历（如果遍历逻辑比较复杂的话），这样就可以省略结构对象
